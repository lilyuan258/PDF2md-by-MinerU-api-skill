...
5. **主动读取相关图片**: 当你读取 `document.md` 时，如果文档内部存在图片链接（例如 `![img](xxx.jpg)`），**你必须主动调用文件读取工具（如 `read_file` 等你的视觉/多模态工具）去读取该输出文件夹下的这些图片文件**。不要仅仅把它们当做纯文本链接忽略过去。如果有需要，可以使用你的目录列出工具（如 `list_directory`）来查看并读取所有提取出的图片。
...
- **强制视觉读取**：转换完成后，所有提取出的图片会和 `document.md` 保存在**同一个文件夹内**。去读取 `document.md` 的同时，**必须解析其中的图片路径，并使用你的工具去读取该目录下的这些图片**。不读取图片就无法实现本工作流“保留图表和复杂排版信息”的核心目的！
